...
public final class Utils {
    ...
    public static ArrayList<Weather> fetchWeatherData(String requestUrl) {
        ...
        ArrayList<Weather> weatherList = extractFeatureFromJson(jsonResponse);
        ...
        try {
            ...
            if (listArray.length() > 0) {
                ...
                for (int i = 0; i < listArray.length(); i++) {
                    ...
                    JSONObject currentFeature = listArray.getJSONObject(i);
                    long dateTime = currentFeature.getLong("dt");

                    JSONObject mainObject = currentFeature.getJSONObject("main");
                    double temperature = mainObject.getDouble("temp");

                    JSONArray weatherArray = currentFeature.getJSONArray("weather");
                    JSONObject weatherObject = weatherArray.getJSONObject(0);
                    String description = weatherObject.getString("main");
                    String icon = weatherObject.getString("icon");

                    JSONObject windObject = currentFeature.getJSONObject("wind");
                    double windSpeed = windObject.getDouble("speed");

                    // Create a new {@link Weather} object
                    weatherList.add(new Weather(dateTime, temperature, description, windSpeed, icon));
                }
            }
